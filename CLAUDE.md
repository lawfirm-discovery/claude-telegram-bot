...
### API 키 위치 (우선순위 순)
1. `/Users/pylon/my_project_v3/.env` — 모든 키의 1차 저장소
2. `/Users/pylon/my_project_v3/crawlers/module/api_key.py` — 크롤러용 키
3. `/Users/pylon/my_project_v3/module/api_key.py` — 백엔드 모듈용 키 (`.env`에서 로드)

API 키가 필요할 때는 **항상 위 파일들을 먼저 확인**할 것. 새 키 발급 전 기존 키 재사용 가능 여부 먼저 검토.
